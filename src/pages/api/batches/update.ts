...
import { updateBatch } from '@/server/db/batch/updateBatch';
import { getServerSession } from 'next-auth/next';
import { z } from 'zod';

class UpdateBatchError extends Error {
    type: string;
    statusCode: number;

    constructor(message: string, type: string, statusCode: number = 400) {
        super(message);
        this.name = 'UpdateBatchError';
        this.type = type;
...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
    ...
    const { eventId, batchId, parentBatch, description, ...batchData } =
        parseResponse.data;
    ...
    try {
        if (startDate > endDate) {
            throw new UpdateBatchError(
                'Start date must be before end date',
                'start_date_after_end_date'
            );
        }

        await updateBatch(eventId, batchId, {
            ...batchData,
        ...
        if (error instanceof UpdateBatchError) {
